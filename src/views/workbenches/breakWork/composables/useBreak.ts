import type { FormPropGroupType, FormPropType } from '@/types';
import { PROCESS_BROKEN } from '@/data';
import { useAuthStore, useSystemStore } from '@/store';
import { utcTime, REGEXP } from '@/utils';
...
import TroubleBrokenCheckForm from '../components/TroubleBrokenCheckForm.vue'
...
export function useBroken() {
  ...
  const parentId = computed(() => businessData.value.exterType || '');
  ...
  const activityProcess = [
    // { id: "start", title: "开始", status: 1 },
    { id: 'userTask1', title: '电力设施产权人提报', status: 1 },
    { id: 'userTask2', title: '执法办经办审核', status: 2 },
    { id: 'userTask3', title: '执法办负责人审核', status: 3 },
    { id: 'userTask4', title: '执法办经办处置', status: 4 },
    // { id: 'userTask5', title: '公安受理反馈', status: 12 },
    { id: 'userTask5', title: '行政立案申请', status: 5 },
    { id: 'userTask6', title: '行政立案调查', status: 6 },
    { id: 'userTask7', title: '行政立案结案', status: 7 },
    // { id: 'userTask9', title: '行政立案结案', status: 9 },
    { id: 'userTask8', title: '电力设施产权人处置', status: 8 }
    // { id: "end", title: "归档", status: 11 }
  ];
    ...
    group: [
      {
        ...
        label: '状态',
        type: 'hidden',
        defaultValue: PROCESS_BROKEN.STEP_2
      },
      {
      ...
      group: [
        ...
        },
        {
          ...
          options: systemStore.dict.external_dispose_way
            .map((item: any) => {
              if (item.label != '移送公安') return item;
            })
            .filter((e: any) => {
              return e != undefined;
            }),
      ...
      group: [
        {
          ...
          type: 'upload',
          rules: [{ required: true }],
          tip: '举报案件登记表注意事项:举报案件登记表是对来访、来电、来函举报、投诉违法行为时，用以记录相关情况的内部文书。填写举报案件登记表应当注意下列事项：1.举报方式：根据举报渠道勾选相应方式。2.举报人：填写举报人的基本情况。3.被举报人：是自然人的，应写明其姓名、性别、年龄和家庭住址，有效证件号码填写身份证号；举报人是法人或其他组织的，应写明其全称、主要办事机构所在地地址、法定代表人或主要负责人的姓名，有效证件号码填写统一社会信用代码。4.举报内容：主要填写举报人反映的违法行为的具体情况，包括违法行为人、时间、地点、违法行为。5.承办部门意见：填写受理部门提出进行调查的意见。如：建议调查核实。6.行政机关分管领导审批意见：填写“同意/不同意调查核实”。',
          uploadConfig: {
        ...
        },
        {
          ...
          type: 'upload',
          rules: [{ required: true }],
          tip: '立案审批表注意事项: 1.案由：处于立案阶段，案由书写形式为“涉嫌+违法行为类别+案”，如“涉嫌擅自设置使用无线电台案”。2.案件来源：包括检查发现、媒体曝光、监督抽检、群众举报或投诉、上级交办、有关部门移送、违法行为人交待等。3.违法事实及立案依据：应当简要叙述初步掌握的违法事实，包括涉嫌违法行为时间、地点、事实、情节等内容，如有检举材料、投诉材料、检测报告等书面材料，可作出说明并附在此表后面。4.承办机构意见：指具体执法处室负责人的审核意见，是否同意立案查处。  5.行政机关负责人审批意见：包括厅机关主要负责人及分管领导。各无线电派出机构由地市无线电管理局领导审批，签发“同意立案”或者“不同意立案”，不能仅以签名代替处理意见。签发日期即为立案日期。6.行政处罚文书多处涉及“单位负责人审批意见”，均参照上述说明填写。',
          uploadConfig: {
      ...
      group: [
        {
          ...
          label: '接受调查通知书',
          type: 'upload',
          tip: '接受调查通知书注意事项: 1.文号填写：本文书文号适用于厅机关执法处室制作的文书，文号书写格式为“闽工信+执法类别+执法类别（执法性质）+〔年份〕+序号”；例如，由节能处制作的文书，文号书写为“闽工信节能调查字〔2020〕第1号”，序号每年的1月1日起从“1”号编起。各设区市无线电管理局制作的文书文号格式建议书写为“设区市简称+无线+执法性质+〔年份〕+序号”，例如“榕无线调查字〔2020〕第1号”。2.涉嫌违反的规定：应填写法律依据名称及条、款、项具体内容。3.携带材料：勾选相应的材料选项，如需携带其他材料，应在“其他”选项中具体列出。4.行政处罚文书涉及文号填写的，均参照上述规定书写。',
          uploadConfig: {
  ...
  const userTask7 = reactive<FormPropGroupType[]>([
    {
      ...
      group: [
        ...
        },
        {
          ...
          type: 'upload-file',
          rules: [{ required: true }],
          tip: '行政处罚事先告知书注意事项: 行政处罚告知书是指在作出行政处罚之前，依法告知当事人给予行政处罚决定的事实、理由、依据以及享有陈述权、申辩权或听证权所制作的通知性文书。 注意事项： 1.行政处罚告知是作出行政处罚决定之前必须履行的一项法定程序，如果下达行政处罚决定前没有履行告知义务，则属于程序违法，行政处罚无效。 2.适用简易程序和一般程序实施行政处罚的案件，都应当有告知程序；告知一般采用书面告知形式。 3.行政处罚事先告知书应当采用阐述式，有说理性的内容，不宜用填空式。4.作出行政处罚决定之前，应当告知当事人依法享有的权利。当事人有权进行陈述和申辩。作出吊销许可证、较大数额罚款等处罚决定时，应告知当事人有要求举行听证的权利。',
          uploadConfig: {
        ...
        },
        {
          ...
          type: 'upload-file',
          rules: [{ required: true }],
          tip: '送达回证注意事项：送达回证是行政执法机关在送达行政执法文书时，记载相关文书已经送达的书面凭证。1.受送达人：填写该文书所送达的当事人的姓名和名称。受送达人为法人或其他组织的，应使用全称；受送达人名称或姓名，应与案件当事人一致。2.送达文件名称、文号：应写明所送达的文书的具体名称及文号。3.受送达人（单位）盖章：由当事人签字或盖章。4.代收人：当事人不在时，由代收人签收，并注明与受送达人的关系。代收人包括同住的成年家属，法人或者其他组织负责收件的人。受送达人（单位）、代收人签收日期为送达日期。5.送达方式:填写“直接送达”“留置送达”“公告送达”“委托送达”或者“邮寄送达”等。邮寄送达要采取特快专递的方式，并将挂号函件收据入卷存档；公告送达应将公告文书归档入卷；留置送达必须有当事人所在地的有关基层组织或者所在单位的代表见证签字；关于送达地址参照《最高法关于进一步加强民事送达工作的若干意见》。6.拒收原因:受送达人或其他法定签收人拒收时，由送达人填写此栏。受送达人拒收的，送达人应在备注栏注明拒收的理由，并请见证人签名或盖章。7.见证人：受送达人拒收时才需要见证人。见证人一般是被邀请的有关基层组织或其所在单位的代表。8.送达人：由两名执法人员签名。',
          uploadConfig: {
        ...
        },
        {
          ...
          type: 'upload-file',
          rules: [{ required: true }],
          tip: '行政处罚决定书注意事项：1.行政处罚决定书应有说理性内容，说明事理、情理和法理。主要包括：（1）对违法行为的构成要件、因果关系和违法事实的认定过程等陈述清楚；（2）阐述证据形式和证据所要证明的内容；（3）适用法律依据时应当完整地引用定性依据和处罚依据；（4）对当事人陈述申辩理由、证据或听证的过程、结论和无线电管理机构是否采纳意见的理由、依据，应当详细阐述，当事人放弃陈述申辩或听证的也应予以说明；（5）作出从重、从轻、减轻或其他有裁量幅度的行政处罚的，应当在行政处罚决定书中说明法定理由和依据。2.当事人提起行政复议的途径有两种方式：一是向具体行政行为发生地的县级人民政府提请行政复议申请，二是以福建省工信厅名义作出的行政处罚决定，复议机关为福建省人民政府或工业和信息化部。各设区市无线电管理局作出的行政处罚决定，复议机关为福建省工业和信息化厅或福建省人民政府。3.本文书要求同样适用于当场行政处罚决定书。',
          uploadConfig: {
        ...
        },
        {
          ...
          label: '行政处罚金额（元）',
          type: 'number',
          display: data => data.sevenPunishWay !== 2,
          rules: [{ required: true }]
        },
      ...
      group: [
        ...
        },
        {
          ...
          rules: [
            ...
            //     if (valueList.join(',').split('，').length < 2)
  ...
  async function customDetailsFormPropsHookFn(
    ...
    const checkValueList: string[] = formData.siteSituation?.split(',');
