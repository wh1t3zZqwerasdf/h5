...
export function currDateArray() {
    return dayjs(new Date()).format("YYYY-MM-DD").split('-');
}

export function getDateString(date: Date, format = 'YYYY-MM-DD') {
...
export function getAPeriodOfTimeDate(type: string) {
    ...
    switch (type) {
        ...
        case 'week':
            ...
            let date = new Date(now.getTime() - (weekKey - 1) * 24 * 3600 * 1000);
            return `${date.getFullYear()}-${
                Number(date.getMonth()) + 1 > 9
                    ? Number(date.getMonth()) + 1
                    : '0' + (Number(date.getMonth()) + 1)
            }-${date.getDate() > 9 ? date.getDate() : '0' + date.getDate()} 00:00:00`;
    }
}
